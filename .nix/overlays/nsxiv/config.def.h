...
/* functions for bypassing <c-x> for key handler */
static bool run_key_handler(const char *, unsigned int);
/* single key binging function */
bool cg_send_key(arg_t key) {
    return run_key_handler(XKeysymToString(key), 0);
}
#define g_send_key { cg_send_key, MODE_ALL }
/* control mask binging function */
...
static const keymap_t keys[] = {
    ...
    /* single key bindings */
    { 0,            XK_z,             g_send_key,           XK_z },
    /* control mask bindings */
    { ControlMask,  XK_w,             g_send_with_ctrl,     XK_w },
    { ControlMask,  XK_l,             g_send_with_ctrl,     XK_l },
    /* normal bindings */
	{ 0,            XK_q,             g_quit,               0 },
